...
export class Floorplan extends go.Diagram {
  ...
  constructor(div: HTMLDivElement | string) {
    ...
    this.addDiagramListener('SelectionDeleted', function (e) {
      const wrt: WallReshapingTool = e.diagram.toolManager.mouseDownTools.elt(
        3
    ...
    this.toolManager.resizingTool.computeMaxSize = function () {
      ...
      if (adornedObject !== null) {
        ...
        if (
          ...
          let stationaryPt: go.Point | null = null;
          let movingPt: go.Point | null = null;
  ...
  get palettes(): Array<go.Palette> {
    return this._palettes;
  }
  set palettes(value: Array<go.Palette>) {
    this._palettes = value;
  }

  // Get / set pointNodes
  get pointNodes(): go.Set<go.Node> {
    return this._pointNodes;
  }
  set pointNodes(value: go.Set<go.Node>) {
    this._pointNodes = value;
  }

  // Get / set dimensionLinks
  get dimensionLinks(): go.Set<go.Link> {
    return this._dimensionLinks;
  }
  set dimensionLinks(value: go.Set<go.Link>) {
    this._dimensionLinks = value;
  }

  // Get / set angleNodes
  get angleNodes(): go.Set<go.Node> {
    return this._angleNodes;
  }
  set angleNodes(value: go.Set<go.Node>) {
  ...
  public enableWallBuilding(): void {
    const fp: Floorplan = this;
    const wallBuildingTool: WallBuildingTool = fp.toolManager.mouseDownTools.elt(
      0
    ) as WallBuildingTool;
    wallBuildingTool.isBuildingDivider = false;
    const wallReshapingTool: WallReshapingTool = fp.toolManager.mouseDownTools.elt(
      3
    ) as WallReshapingTool;
  ...
  public changeUnits(form: HTMLFormElement): void {
    ...
    const unitsConversionFactorInput: HTMLInputElement = document.getElementById(
    ...
    const newUnitsConverstionFactor: number = fp.convertUnits(
      prevUnits,
      units,
      oldUnitsConversionFactor
    );
    fp.model.setDataProperty(
      fp.model.modelData,
      'unitsConversionFactor',
      newUnitsConverstionFactor
    );
   ...
   */
  public changeUnitsConversionFactor(
    unitsConversionFactorInput: HTMLInputElement,
  ...
  public updateAllRoomBoundaries(changedWalls: go.Set<go.Group>): void {
    ...
    rooms.iterator.each(function (r: go.Node) {
      ...
      while (!boundsFound && !triedAllIntersections) {
        ...
        if (e1 !== null && e2 !== null) {
          w1 = fp.findNodeForKey(e1[0]) as go.Group;
          s1 = e1[1];
          w2 = fp.findNodeForKey(e2[0]) as go.Group;
          s2 = e2[1];
        } else {
  ...
  public maybeAddRoomNode(
    ...
    const rooms: go.Iterator<go.Node> = fp.findNodesByExample({
      category: 'RoomNode',
  ...
  public getRoomWalls(pt: go.Point): Array<any> | null {
      ...
      ccWalls.iterator.each(function (w: go.Group) {
        ...
        if (
          ...
          if (path === undefined || path === null) {
            ...
            const ip1: go.Point | null = fp.getLinesIntersection(
              pt,
              oPt,
              wall.data.smpt1,
              wall.data.empt1
            );
            const ip2: go.Point | null = fp.getLinesIntersection(
              pt,
              oPt,
              wall.data.smpt2,
              wall.data.empt2
    ...
    for (let i: number = 0; i < wallsDistArr.length; i++) {
      ...
      if (polygon !== null) {
        ...
        for (let j: number = 0; j < path.length; j++) {
          ...
          if (
            fp.isPointInPolygon(polygon.toArray(), ept) ||
    ...
    if (roomOuterBoundaryPts !== null) {
      ...
      const newRoomBoundaryWalls: Array<any> = fp.addInternalWallsToRoom(
  ...
  public addInternalWallsToRoom(
     ...
     */
    function recursivelyFindInteralClusterPath(
      wall: go.Group,
      ...
      if (!handledWalls.contains(ow)) {
        ...
        if (ip !== null) {
          ...
          if (bw1 !== null && bw2 !== null) {
            let p = recursivelyFindInteralClusterPath(
              bw1,
              intersectionWalls,
              ip,
              seenIp,
  ...
  public getClockwiseWallEndpoint(w: go.Group, side: number): go.Point | null {
    ...
    let outsidePt: go.Point | null = null;
  ...
  public findRoomHoles(roomBoundaryWalls: Array<any>, pt: go.Point): Array<Array<any>> {
    ...
    const offendingWalls: go.Set<go.Group> = new go.Set<go.Group>();
    const roomOuterBoundaryPts: go.List<go.Point> | null =
      fp.makePolygonFromRoomBoundaries(roomBoundaryWalls);
    walls.iterator.each(function (w: go.Group) {
      const s: go.Point = w.data.startpoint;
      const e: go.Point = w.data.endpoint;
      if (
    ...
    function recursivelyFindHolePath(
      ...
      // have we come full circle?
      const isNextClockwiseWallOrigWall: boolean =
        iw.count > 1 && iw.toArray()[1].data.key === origWall.data.key;

      const atFinalIp: boolean =
        (isNextClockwiseWallOrigWall && (w.data.key !== origWall.data.key || isSoloWallHole)) ||
  ...
  public isPointInRoom(r: go.Node, p: go.Point): boolean {
    ...
    if (r === null || r === undefined || !(r instanceof go.Node) || r.data.category !== 'RoomNode')
      return false;
    const polygon: go.List<go.Point> | null = fp.makePolygonFromRoomBoundaries(
  ...
  public updateWallDimensions(wallsToDisplayFor?: go.Set<go.Group>): void {
    ...
    selection.iterator.each(function (part: go.Part) {
      if (
        (part.category === 'WindowNode' || part.category === 'DoorNode') &&
        part.containingGroup !== null
      ) {
        walls.add(part.containingGroup);
      }
      if (
        part.category === 'WallGroup' &&
        part.data &&
      ...
      for (let i = 1; i < 3; i++) {
        ...
        wall.memberParts.iterator.each(function (wallPart) {
          if (wallPart.isSelected) {
            ...
            const newEp1: go.Point = floorplan.rotateAndTranslatePoint(
              ep1,
              angle + 0,
              wall.data.thickness / 2
            );
            const newEp2: go.Point = floorplan.rotateAndTranslatePoint(
              ep2,
              angle + 0,
              wall.data.thickness / 2
            );
        ...
        for (let j: number = 0; j < wallPartEndpoints.length - 1; j++) {
          ...
          if (linkPoint1 !== null && linkPoint2 !== null) {
            const newLoc1: go.Point = floorplan.getAdjustedPoint(
              wallPartEndpoints[j].copy(),
              wall,
              angle,
              15
            );
            const newLoc2: go.Point = floorplan.getAdjustedPoint(
  ...
  public updateWallAngles() {
    ...
    if (floorplan.model.modelData.preferences.showWallAngles) {
      ...
      for (let i: number = 0; i < selectedWalls.length; i++) {
        ...
        const possibleWalls: go.Iterator<go.Group> = floorplan.findNodesByExample({
          category: 'WallGroup',
        }) as go.Iterator<go.Group>;
        ...
        possibleWalls.iterator.each(function (otherWall) {
          ...
          if (
            ...
            const intersectionPoint: go.Point | null = floorplan.getWallsIntersection(
              wall,
              otherWall
            );
            if (intersectionPoint !== null) {
              const wrt: WallReshapingTool = floorplan.toolManager.mouseDownTools.elt(
                3
              ) as WallReshapingTool;
              ...
              wallsInvolved.iterator.each(function (w) {
                const tolerance: number =
                  floorplan.model.modelData.gridSize >= 10
  ...
  return $(go.Adornment,
    ...
    $(go.TextBlock,
      ...
      new go.Binding('text', '', function (text, obj) {
        ...
        const name =
          obj.part.adornedObject.category === 'MultiPurposeNode' ? data.text : data.caption;
        return (
          'Name: ' +
          name +
          '\nNotes: ' +
...
function makeAngleNode() {
  ...
  return $(go.Node,
    ...
    new go.Binding('location', 'loc', go.Point.parse).makeTwoWay(go.Point.stringify),
    $(go.Shape,
      'Circle', // placed where walls intersect, is invisible
...
function makeFurnitureRotateAdornmentTemplate() {
  ...
  return $(go.Adornment,
    $(go.Shape,
      'Circle',
      { cursor: 'pointer', desiredSize: new go.Size(7, 7), fill: 'lightblue', stroke: 'dodgerblue' }
...
function makeMultiPurposeNode() {
  ...
  return $(go.Node,
    ...
    $(go.Panel,
      ...
      $(go.Shape, 'RoundedRectangle', { fill: 'beige', opacity: 0.5, stroke: null }),
      $(go.TextBlock,
        {
          margin: 5,
          wrap: go.Wrap.Fit,
  ...
  return $(go.Adornment,
    ...
    $(go.Panel,
      ...
      $('Button',
        ...
        $(go.Shape,
          'TriangleLeft',
          { desiredSize: new go.Size(10, 10), name: 'BUTTONSHAPE' },
      ...
      $(go.Panel,
        'Horizontal',
        {
          ...
          itemTemplate: $('Button',
            ...
            $(go.Picture,
              { name: 'BUTTON_IMAGE' },
              new go.Binding('source', '', function (str: any, b: any) {
  ...
  return $(go.Node,
    ...
    $(go.Panel,
      ...
      $(go.Panel,
        ...
        $(go.Shape,
          'RoundedRectangle',
          { fill: 'beige', opacity: 0.5, stroke: null, strokeWidth: 3, name: 'ROOM_LABEL_SHAPE' },
        ...
        $(go.Panel,
          ...
          $(go.TextBlock,
            'Room Name',
            { editable: true, cursor: 'text', font: 'normal normal bold 13px sans-serif' },
...
function makeDoorSelectionAdornment() {
  ...
  return $(go.Adornment,
    ...
    $(go.Panel, 'Auto', $(go.Shape, { fill: null, stroke: null }), $(go.Placeholder)),
    $(go.Panel,
      'Horizontal',
      { defaultStretch: go.Stretch.Vertical },
      $('Button',
        $(go.Picture,
          { source: 'images/flipDoorOpeningLeft.png', column: 0, desiredSize: new go.Size(12, 12) },
...
function makeWindowNode() {
  ...
  return $(go.Node,
    ...
    $(go.Shape,
      ...
      new go.Binding('width', 'length', function (width, obj) {
        return width - 10;
...
const FURNITURE_NODE_DATA_ARRAY = [
  ...
  },
  {
    ...
    type: 'Sink',
    geo:
      'F1 M0 0 L40 0 40 40 0 40 0 0z M5 7.5 L18.5 7.5 M 21.5 7.5 L35 7.5 35 35 5 35 5 7.5 M 15 21.25 A 5 5 180 1 0 15 21.24' +
  ...
  },
  {
    ...
    type: 'Double Sink',
    geo:
      'F1 M0 0 L75 0 75 40 0 40 0 0 M5 7.5 L35 7.5 35 35 5 35 5 7.5 M44 7.5 L70 7.5 70 35 40 35 40 9' +
  ...
  },
  {
    ...
    type: 'Staircase',
    geo:
      'F1 M0 0 L 0 100 250 100 250 0 0 0 M25 100 L 25 0 M 50 100 L 50 0 M 75 100 L 75 0' +
  ...
  },
  {
    ...
    type: 'Dining Table',
    geo:
      'F1 M 0 0 L 0 100 200 100 200 0 0 0 M 25 0 L 25 -10 75 -10 75 0 M 125 0 L 125 -10 175 -10 175 0' +
